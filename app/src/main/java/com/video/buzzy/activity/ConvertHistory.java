...
import com.video.buzzy.databinding.ActivityConvertHistoryBinding;
...
public class ConvertHistory extends AppCompatActivity {

    ActivityConvertHistoryBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_convert_history);
    ...
    private void initView() {
        ...
        binding.rvCoinHistory.setAdapter(viewModel.coinHistoryAdapter);

        sessionManager = new SessionManager(this);

        binding.rvCoinHistory.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            ...
            public void onResponse(Call<CoinHistoryRoot> call, Response<CoinHistoryRoot> response) {
                if (response.code() == 200 && response.isSuccessful()) {
                    if (!response.body().getHistory().isEmpty()) {
                        ...
                        viewModel.coinHistoryAdapter.addData(response.body().getHistory());
                        isLoding = false;
                    } else if (start == 1) {
