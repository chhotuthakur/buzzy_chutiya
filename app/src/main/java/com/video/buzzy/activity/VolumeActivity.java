...
import com.video.buzzy.workers.MergeAudioVideoWorker2;
import com.video.buzzy.workers.MergeAudiosWorker;
        ...
        wm.getWorkInfoByIdLiveData(request2.getId())
                .observe(this, info -> {
                    ...
                    if (info.getState() == WorkInfo.State.SUCCEEDED) {
                        ...
                        Log.d(TAG, "submitForMerge: success");
                        if (getResources().getBoolean(R.bool.skip_pan_audio_screen)) {
                            Log.d(TAG, "submitForMerge: filter");
                            proceedToFilter(merged2);
                        } else {
                            Log.d(TAG, "submitForMerge: pan");
