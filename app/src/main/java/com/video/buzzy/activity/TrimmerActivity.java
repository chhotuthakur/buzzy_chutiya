...
import com.video.buzzy.activity.trimmer.TrimTimeBar;
...
import com.video.buzzy.workers.VideoTrimmerWorker3;
...
public class TrimmerActivity extends BaseActivity implements AnalyticsListener, TrimTimeBar.Listener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTrimStartTime = 0;
        mTrimEndTime = Math.min(mDuration, (int) Const.MAX_DURATION);
    ...
    private void commitSelection() {
        ...
        if (duration > Const.MAX_DURATION) {
            String message = getString(
                    R.string.message_trim_too_long,
                    TimeUnit.MILLISECONDS.toSeconds(Const.MAX_DURATION)
            );
            Toast.makeText(this, message, Toast.LENGTH_SHORT).show();
        } else if (duration < Const.MIN_DURATION) {
            String message = getString(
                    R.string.message_trim_too_short,
                    TimeUnit.MILLISECONDS.toSeconds(Const.MIN_DURATION)
            );
            Toast.makeText(this, message, Toast.LENGTH_SHORT).show();
        } else {
            submitForTrim();
