...
import com.video.buzzy.databinding.ItemSongBinding;
...
public class SongsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public class SongsViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(int position) {
            SongRoot.SongItem songs = songItems.get(position);
            Glide.with(binding.getRoot()).load(BuildConfig.BASE_URL + songs.getImage()).into(binding.imgSong);
            binding.title.setText(songs.getTitle());
            binding.info.setText(songs.getSinger());
            binding.getRoot().setOnClickListener(v -> onSongClickListner.onSongClick(songs));
        }
    }
}
